...
更新时间：2026-03-20 16:28 +08:00

## 1. 项目定位

- 当前分支：`dev`
- 当前阶段：Portfolio Monitoring Core Alpha Frozen
- 当前目标：冻结当前 core 实现，进入真实使用验证期
- 当前范围：
  - 基金 / 指数 / ETF 监控
  - 单机 SQLite 持久化
  - 单次运行编排
...
- [x] 组合摘要 / 持仓列表 / 组合报告读取面
- [x] 组合级规则设计首版（scope + portfolio metrics）
- [x] 组合回撤 / 仓位分布指标
- [x] 核心单元测试
...
- 不查库也能直接看到组合层面的核心指标
- 能直接看到组合峰值、当前回撤和仓位集中度
- 已记录持仓的资产可以按组合作为一个整体被查看

### 阶段结论

- `M1` 到 `M6` 已完成，当前 `Core Alpha` 收口
- 从现在开始进入真实使用验证期，不再继续主动扩展 core 功能面
- 冻结期内只接受三类变更：
  - 真实运行暴露的 bug 修复
  - 阻碍使用的 CLI / 数据问题修复
  - 阻断下一阶段的结构性修复
